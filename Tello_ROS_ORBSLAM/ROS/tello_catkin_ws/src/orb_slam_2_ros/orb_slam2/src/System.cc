			   ...
			   mbDeactivateLocalizationMode(false), map_file(map_file), load_map(load_map)
{
    ...
    mpPointCloudMapping = make_shared<PointCloudMapping>( 0.01,50,1.0 );
    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    mpTracker = new Tracking(mpPointCloudMapping,this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
...
void System::setPointCloudCyOffset(double offset){
    mpPointCloudMapping->setCyOffset(offset);
}
