...
Node::Node (ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  node_handle_.param<double>(name_of_node_+"/point_cloud_offset",point_cloud_cy_offset,0.0); //没用了
  node_handle_.param<double>(name_of_node_+"/initialization_pitch_degree",initialization_pitch_degree,0.0);
  telloInitPose = cv::Mat::eye(4,4,CV_32F);
  if (fabs(initialization_pitch_degree) > eps){
    double degree = initialization_pitch_degree *M_PI / 180 ;
    telloInitPose.at<float>(1,1) = cos(degree);
    telloInitPose.at<float>(1,2)= -sin(degree);
    telloInitPose.at<float>(2,1) = sin(degree);   
    telloInitPose.at<float>(2,2) =cos(degree) ; 
  }
