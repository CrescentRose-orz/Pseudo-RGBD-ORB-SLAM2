...
class TelloSlamControler(object):
    ...
    def __init__(self):
        ...
        #added for  command tello with real path
        self.planed_path_publisher = rospy.Publisher(self.publish_prefix+'planed_real_path', Path, queue_size = 1)
        ...
        #change path_planner output to real world path
        rospy.Subscriber('/waypoints',Path , self.planed_path_callback)
    ...
    def rotate_point(self, point, alpha):
        ...
        return out_point
#[todo]
    def slamPose2realPose(self,slam_pose):
        real_world = slam_pose
        #real_world.pose.orientation = slam_pose.pose.orientation
        rotated_pos = slam_pose.pose.position
        real_world.pose.position.x = (rotated_pos.x - self.rotated_position_0.x)* self.real_world_scale
        real_world.pose.position.y = (rotated_pos.y - self.rotated_position_0.y)* self.real_world_scale
        real_world.pose.position.z = (rotated_pos.z - self.rotated_position_0.z)* self.real_world_scale
       # real_world.pose.orientation = slam_pose.pose,orientation
        return real_world

    def planed_path_callback(self,slamPath):
        for pose in slamPath.poses:
            pose = self.slamPose2realPose(pose)
        self.planed_path_publisher.publish(slamPath)
        return 
