...
class TelloUI(object):
    ...
    def __init__(self, root):
        ...
        #whether traj thread is running?
        self.thread_begin = False
       ...
       # self.panel_for_pose_handle_show = None
        ...
        #self.angle = 12.0
        #compensate in orbslam2 (initial pose)
        self.angle = 0
        ...
        self.trajectory_threshold = Point(0.4, 0.4, 1)
        self.trajectory_threshold = Point(1,1,3)
        ...
        rospy.Subscriber(self.publish_prefix+'planed_real_path',Path, self.planner_trajectory_receive_callback)
...
#[todo]
    def planner_trajectory_receive_callback(self,path):
        print("get real path! len:",len(path.poses) )
        self.lock.acquire()
        self.trajectory_list=[]
        for stamped_pose in path.poses:
            ...
            y = pose.position.y
            z = pose.position.z 
            yaw =  self.quatenrion_point_to_euler_degree(pose.orientation).z
            point = []
            point.append(x)
            point.append(y)
            point.append(z)
        ...
        self.lock.release()
        if ~self.thread_begin:
            self.trajectory_kill = False
            self.land = False
            self.thread_begin = True
